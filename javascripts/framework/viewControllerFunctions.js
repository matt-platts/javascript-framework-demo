...
page.prototype.makePageCallsInner = function(){
	...
	if(viewingAsId==currentUserVar || pageIdentifier==_homePage || pageIdentifier == _loginPage){
		...
		for(layoutSections in pageRegister[currentPageIdentifier]["blocks"]) {
			...
			if(pageRegister[previousPageIdentifier]["blocks"].hasOwnProperty(layoutSections)){
				...
				for(sectionsBlock in pageRegister[currentPageIdentifier]["blocks"][layoutSections]) {
					...
					if(sectionsBlock=="parentSignupAside" || sectionsBlock=="progressBar" || sectionsBlock=="logoHeaders" || sectionsBlock=="mainHeadings" || sectionsBlock=="topAccountDetails" || sectionsBlock=="webappHeader"){
	...
	for (callID in this.currentPageAjaxCalls) {
		...
		var AJAXcall = $.ajax({
			...
			error: function(jqXHR, textStatus, errorThrown){
				...
				//check if 401 - this means that an authorization is required, (user not logged in / the session has timed out), so forward the user to the login page
				if(jqXHR.status==401){

					// here, check that the page user type is not 'external'.. swap this for array lookup rather than hard coded text..
					if (pageRegister[currentPageIdentifier]['allowedUserTypes']!="external"){
	...
	if(!calls){
		__page.noAJAXRender(pageIdentifier);
	}
...
*/
page.prototype.noAJAXRender = function(pageIdentifier) {
	var section;
	for(section in pageRegister[pageIdentifier].blocks) {
		if($.inArray(section, this.sectionsRendered) == -1){
...
page.prototype.layoutRender = function(layoutIdentifier) {
	...
	document.getElementById(_view_container).innerHTML = new EJS({
